...
import { UserProfile, LIFETIME_BASELINE_CUTOFF } from "../types";
import { 
...
export const AdminDashboard: React.FC = () => {
  ...
  // Active / Archive reset & paid modal state
  const [resetConfirmEmployee, setResetConfirmEmployee] = useState<any | null>(null);
  const [paidConfirmEmployee, setPaidConfirmEmployee] = useState<any | null>(null);
  ...
  // Trigger Paid Operation
  const triggerPaidFlow = (emp: any) => {
    setPaidConfirmEmployee(emp);
  };

  const handleConfirmPaid = async () => {
    if (!paidConfirmEmployee) return;
    setActionLoading(true);
    setError(null);

    try {
      const empId = paidConfirmEmployee.user_id;
      const employeeExpenses = expenses.filter(e => e.employee_id === empId && !e.is_archived);
      
      if (employeeExpenses.length === 0) {
        throw new Error("This employee has no active unpaid expenses to mark as paid.");
      }

      const activeSumVal = Math.round(
        employeeExpenses.reduce((s, e) => s + parseFloat(e.amount || 0), 0) * 100
      ) / 100;
      const now = new Date();
      const currentMonthVal = now.getMonth() + 1;
      const currentYearVal = now.getFullYear();
      const paymentTitle = `Paid - ${now.toLocaleString("default", { month: "long" })} ${currentYearVal}`;

      // Call backend API with service role credentials
      let paymentSuccess = false;
      try {
        const res = await fetch("/api/admin/mark-employee-paid", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${sessionToken}`
        ...
        console.warn("API mark-employee-paid fallback to client SDK", e);
      }
      ...
      if (!paymentSuccess) {
        ...
        if (archError) throw new Error("Could not initialize payment archive: " + archError.message);
        ...
        if (updateError) {
          ...
          throw new Error("Could not mark expenses as paid: " + updateError.message);
        }
      }

      setPaidConfirmEmployee(null);
      triggerSuccessMsg(`Payment of ₹${activeSumVal.toLocaleString("en-IN", { minimumFractionDigits: 2 })} recorded! Outstanding balance set to ₹0.`);
      await loadDashboardData();
  ...
  // Excel detailed individual expense records exporter for all staff
  const handleExportStaff = () => {
    ...
    employees.forEach((emp) => {
      ...
      if (empExps.length === 0) {
        rows.push({
          ...
          "Account Status": emp.is_active ? "Active" : "Deactivated",
          "Expense Date": "—",
          "Expense Type": "—",
      ...
      const sortedExps = [...empExps].sort((a, b) => {
      ...
      sortedExps.forEach((exp) => {
        const paymentStatus = exp.is_archived ? "Paid" : "Unpaid";
        const expenseType = exp.expense_type === "Other" && exp.custom_type 
          ? `Other (${exp.custom_type})` 
          : (exp.expense_type || "General");

        let description = exp.notes || "";
        if (exp.expense_type === "Petrol") {
          const petrolDetails = [
            exp.from_location && exp.to_location ? `${exp.from_location} to ${exp.to_location}` : "",
            exp.distance_km ? `${exp.distance_km} km` : "",
            exp.vehicle_type || ""
          ].filter(Boolean).join(", ");
          if (petrolDetails) {
            description = description ? `${description} (${petrolDetails})` : petrolDetails;
          }
        } else if (exp.expense_type === "Food") {
          const foodDetails = [
            exp.meal_type || "",
            exp.num_individuals ? `${exp.num_individuals} pax` : "",
            exp.individual_names || ""
          ].filter(Boolean).join(", ");
          if (foodDetails) {
            description = description ? `${description} (${foodDetails})` : foodDetails;
          }
        }
        ...
        rows.push({
          ...
          "Amount (INR)": parseFloat(exp.amount || 0),
          "Payment Status": paymentStatus,
          "Approval Status": exp.workflow_status || "Pending Accounts Approval",
          "Expense Nature": exp.expense_nature || "Reimbursement",
          "GST Selection": exp.gst_type || "Non-GST Bill",
          "Payment Term": exp.payment_term || "Immediate",
          "Receipt / Proof URL": exp.proof_url || "—",
          "Submission Date": exp.created_at ? new Date(exp.created_at).toLocaleString() : ""
        });
      });
    });
    ...
    const worksheet = XLSX.utils.json_to_sheet(rows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Employee Expense Ledger");
    const today = new Date().toISOString().split("T")[0];
    XLSX.writeFile(workbook, `employee-expenses-detailed-ledger-${today}.xlsx`);
  };
  ...
  // Preserves existing lifetime total baseline, and for claims here onwards only includes once paid by admin
  const lifetimeTotalAcrossAll = Math.round(
    expenses
      .filter((e) => {
        const isBaseline = !e.created_at || e.created_at <= LIFETIME_BASELINE_CUTOFF;
  ...
  const getEmployeeStats = (empUserId: string) => {
    ...
    // Lifetime total: keeps current baseline value, and for further claims only adds once paid by admin
...
ADD COLUMN IF NOT EXISTS num_individuals INT,
ADD COLUMN IF NOT EXISTS individual_names TEXT,
ADD COLUMN IF NOT EXISTS actual_bill_amount NUMERIC(12, 2);
...
NOTIFY pgrst, 'reload schema';`}
              ...
              <table className="min-w-full text-left text-xs divide-y divide-slate-150 text-slate-700">
                <thead className="bg-slate-50 font-bold uppercase text-[10px] text-slate-500 tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4">Total Unclaimed Amount</th>
                          ...
                          <div className="inline-flex gap-2 justify-end">
                            {isClaimant && (
                              <>
                                ...
                                <button
                                  onClick={() => triggerPaidFlow(emp)}
                                  disabled={stats.activeTotal <= 0 && stats.monthTotal <= 0}
                                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white font-bold text-[10px] uppercase transition shadow-2xs cursor-pointer"
                                  title="Mark outstanding expenses as Paid for this employee"
                                >
                      ...
                      <tr key={item.id} className="hover:bg-slate-50/50 transition-colors align-top">
                        ...
                        <td className="px-6 py-4 max-w-xs whitespace-normal">
                          <div className="flex flex-col gap-1.5">
                            ...
                            {item.expense_type === "Petrol" && item.from_location && (
                              <div className="bg-slate-50 p-2 rounded-lg border border-slate-100 text-[10px] text-slate-600 space-y-0.5 font-sans mt-1">
                                <strong className="text-slate-700 block text-[10px] mb-0.5">Petrol Route:</strong>
                                <p><span className="text-slate-400">Route:</span> <strong>{item.from_location}</strong> to <strong>{item.to_location}</strong></p>
                                <p><span className="text-slate-400">Vehicle:</span> <strong>{item.vehicle_type}</strong> • <span className="text-slate-400">Dist:</span> <strong>{item.distance_km} km</strong></p>
                              </div>
                            )}
                            {item.expense_type === "Food" && item.meal_type && (
                              <div className="bg-slate-50 p-2 rounded-lg border border-slate-100 text-[10px] text-slate-600 space-y-0.5 font-sans mt-1">
                                <strong className="text-slate-700 block text-[10px] mb-0.5">Food Expense details:</strong>
                                <p><span className="text-slate-400">Meal:</span> <strong>{item.meal_type}</strong> • <span className="text-slate-400">People:</span> <strong>{item.num_individuals}</strong></p>
                                {item.individual_names && <p><span className="text-slate-400">Names:</span> <strong>{item.individual_names}</strong></p>}
                                <p><span className="text-slate-400">Bill Entered:</span> <strong>₹{parseFloat(item.actual_bill_amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</strong></p>
                              </div>
                            )}
                            {item.proof_url ? (
                              <a
      ...
      {/* CONFIRM PAID OVERLAY WINDOW */}
      {paidConfirmEmployee && (() => {
        const empPendingExps = expenses.filter(e => e.employee_id === paidConfirmEmployee.user_id && !e.is_archived);
        const totalPending = Math.round(empPendingExps.reduce((s, e) => s + parseFloat(e.amount || 0), 0) * 100) / 100;
          ...
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white max-w-md w-full rounded-2xl border border-slate-200 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
              ...
              <div className="p-6 bg-emerald-50 border-b border-emerald-100 flex gap-4">
                <div className="h-10 w-10 shrink-0 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700">
                ...
                <div>
                  ...
                  <p className="text-xs text-emerald-800 font-medium mt-1">
                    Mark employee's outstanding expenses as reimbursed and settled.
                  </p>
                </div>
              </div>
              ...
              <div className="p-6 space-y-4">
                <p className="text-sm text-slate-700 font-medium leading-relaxed">
                  Are you sure you want to mark <span className="font-mono font-black text-emerald-700 text-base">₹{totalPending.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span> as paid for <span className="font-bold text-slate-950">{paidConfirmEmployee.name}</span>?
                </p>

                <div className="p-3.5 bg-slate-50 rounded-xl text-xs text-slate-600 border border-slate-200 font-medium leading-relaxed space-y-2">
                  <p className="flex items-center gap-2 text-slate-700">
                    <Check className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>Unclaimed balance for this employee will become <strong className="text-emerald-700">₹0.00</strong>.</span>
                  </p>
                  <p className="flex items-center gap-2 text-slate-700">
                    <Check className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>All expense records, original amounts, and receipts remain permanently preserved in history.</span>
                  </p>
                  <p className="flex items-center gap-2 text-slate-700">
                    <Check className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>Lifetime total spend will remain completely intact.</span>
                  </p>
                </div>
              </div>
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-150 flex justify-end gap-3 font-semibold">
                <button
                  onClick={() => setPaidConfirmEmployee(null)}
                  className="px-4 py-2 text-xs text-slate-600 hover:text-slate-900 rounded cursor-pointer transition"
                >
                  Cancel
                </button>
                <button
                  onClick={handleConfirmPaid}
                  disabled={actionLoading || totalPending <= 0}
                  className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-xs px-5 py-2.5 rounded-lg shadow-sm flex items-center gap-1.5 cursor-pointer transition"
                >
                  {actionLoading ? "Processing Payment..." : "Confirm & Mark as Paid"}
                </button>
              </div>
