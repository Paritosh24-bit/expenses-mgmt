...
import { LIFETIME_BASELINE_CUTOFF } from "../types";
...
export const EmployeeDetails: React.FC = () => {
  ...
  const handleExportEmployeeExpenses = () => {
    ...
    const rows = expenses.map((item) => {
      ...
      if (item.expense_type === "Petrol") {
        const petrolDetails = [
          item.from_location && item.to_location ? `${item.from_location} to ${item.to_location}` : "",
      ...
      } else if (item.expense_type === "Food") {
        const foodDetails = [
          item.meal_type || "",
          item.num_individuals ? `${item.num_individuals} pax` : "",
          item.individual_names || ""
        ].filter(Boolean).join(", ");
        if (foodDetails) {
      ...
      return {
        ...
        "Expense Type": item.expense_type === "Other" && item.custom_type ? `Other (${item.custom_type})` : item.expense_type,
        "Description": description || "—",
        "Amount (INR)": parseFloat(item.amount || 0),
        "Payment Status": item.is_archived ? "Paid" : "Unpaid",
    ...
    XLSX.writeFile(workbook, `${sanitizedName}-expenses-detailed-${today}.xlsx`);
  };
  ...
  // Calculated Lifetime total: maintains existing baseline value, and for further claims only adds once paid by admin
  ...
  // Handle Paid & Reimbursement operation logic:
  const handleArchiveAndReset = async () => {
    ...
    try {
      ...
      if (activeRows.length === 0) {
        throw new Error("This employee does not have any active, unpaid expense entries at this time.");
      }
      ...
      const defaultName = `Paid - ${now.toLocaleString("default", { month: "long" })} ${archiveYearValue}`;
      const paymentTitle = (resetName.trim() || defaultName);
      ...
      if (!apiSucceeded) {
        ...
        if (archError) throw new Error("Could not record payment archive: " + archError.message);

        // Step 3: Update active items
        ...
        if (updateError) {
          ...
          throw new Error("Could not update expenses status: " + updateError.message);
        }
      }

      setResetSuccess(`Payment of ₹${activeAmountSum.toLocaleString("en-IN", { minimumFractionDigits: 2 })} recorded! Unclaimed balance set to ₹0.`);
      ...
      alert(err.message || "Payment trigger failed.");
    } finally {
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            disabled={activeTotal <= 0 && currentMonthTotal <= 0}
            className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white font-bold text-xs px-5 py-3 rounded-lg shadow-sm transition inline-flex items-center gap-2 cursor-pointer"
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white max-w-md w-full rounded-2xl border border-slate-200 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="bg-emerald-50 border-b border-emerald-100 p-6 flex gap-4">
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-slate-700 font-medium leading-relaxed">
                Are you sure you want to mark <span className="font-mono font-black text-emerald-700 text-base">₹{activeTotal.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span> as paid for <span className="font-bold text-slate-950">{profile.name}</span>?
              </p>
              ...
              <div className="p-3.5 bg-slate-50 rounded-xl text-xs text-slate-600 border border-slate-200 font-medium leading-relaxed space-y-2">
                <p className="flex items-center gap-2 text-slate-700">
                  ...
                  <span>Unclaimed balance will become <strong className="text-emerald-700">₹0.00</strong>.</span>
                </p>
                <p className="flex items-center gap-2 text-slate-700">
                  <Check className="w-4 h-4 text-emerald-600 shrink-0" />
                  <span>All individual bill logs and uploaded PDF attachments remain permanently preserved.</span>
                </p>
                <p className="flex items-center gap-2 text-slate-700">
                  <Check className="w-4 h-4 text-emerald-600 shrink-0" />
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3.5">
              <button
                ...
                className="px-4 py-2 text-xs font-semibold text-slate-600 hover:text-slate-950 transition cursor-pointer"
              >
                Cancel
              </button>
              ...
              <button
                ...
                disabled={isResetPending || activeTotal <= 0}
                className="inline-flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-xs px-5 py-2.5 rounded-lg shadow-xs cursor-pointer transition"
              >
                {isResetPending ? "Recording Payment..." : "Confirm & Mark as Paid"}
              </button>
            </div>
          </div>
        </div>
