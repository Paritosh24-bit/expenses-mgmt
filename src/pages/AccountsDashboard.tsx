                  ...
                  <tr key={exp.id} className="hover:bg-slate-50/20 transition-all font-sans">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        ...
                        {/* Inline Petrol Details */}
                        {exp.expense_type === "Petrol" && exp.from_location && (
                          <div className="text-[9.5px] text-slate-500 mt-1 space-y-0.5 bg-slate-50 p-1.5 rounded border border-slate-150">
                            <p className="font-semibold text-slate-700">🚗 Route Detail:</p>
                            <p>{exp.from_location} to {exp.to_location}</p>
                            <p className="font-mono text-[9px] text-indigo-600">{exp.vehicle_type} • {exp.distance_km} km</p>
                          </div>
                        )}

                        {/* Inline Food Details */}
                        {exp.expense_type === "Food" && exp.meal_type && (
                          <div className="text-[9.5px] text-slate-500 mt-1 space-y-0.5 bg-slate-50 p-1.5 rounded border border-slate-150">
                            <p className="font-semibold text-slate-700">🍽️ Food Detail:</p>
                            <p className="font-bold text-slate-600">{exp.meal_type} for {exp.num_individuals} pax</p>
                            {exp.individual_names && <p className="text-slate-400">Names: {exp.individual_names}</p>}
                            <p className="font-mono text-[9px] text-emerald-600">Bill: ₹{parseFloat(exp.actual_bill_amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</p>
                          </div>
                        )}
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full overflow-hidden border border-slate-205 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
                ...
                {reviewItem.expense_type === "Petrol" && reviewItem.from_location && (
                  <div className="pt-2 border-t border-slate-200 text-slate-600 mt-1 space-y-1 font-sans">
                    <span className="font-bold block text-slate-750">Petrol / Mileage Details:</span>
                    <p className="mt-0.5"><span className="text-slate-400 font-medium">Route:</span> <strong className="text-slate-700">{reviewItem.from_location}</strong> to <strong className="text-slate-700">{reviewItem.to_location}</strong></p>
                    <p><span className="text-slate-400 font-medium">Vehicle Type:</span> <strong className="text-slate-700">{reviewItem.vehicle_type}</strong></p>
                    <p><span className="text-slate-400 font-medium">Distance:</span> <strong className="text-slate-700">{reviewItem.distance_km} km</strong></p>
                  </div>
                )}
                {reviewItem.expense_type === "Food" && reviewItem.meal_type && (
                  <div className="pt-2 border-t border-slate-200 text-slate-600 mt-1 space-y-1 font-sans">
                    <span className="font-bold block text-slate-750">Food Allowance Details:</span>
                    <p className="mt-0.5"><span className="text-slate-400 font-medium">Meal Category:</span> <strong className="text-slate-700">{reviewItem.meal_type}</strong></p>
                    <p><span className="text-slate-400 font-medium">People Count:</span> <strong className="text-slate-700">{reviewItem.num_individuals}</strong> {reviewItem.individual_names ? `(${reviewItem.individual_names})` : ""}</p>
                    <p><span className="text-slate-400 font-medium">Actual Bill Amount:</span> <strong className="text-slate-700">₹{parseFloat(reviewItem.actual_bill_amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</strong></p>
                  </div>
                )}
              </div>
