...
app.post("/api/expenses/submit", requireUser, async (req: any, res: any) => {
  try {
    ...
    const { 
      ...
      from_location, to_location, distance_km, vehicle_type,
      meal_type, num_individuals, individual_names, actual_bill_amount
    } = req.body;
    ...
    const expenseObj = {
      ...
      num_individuals: num_individuals ? parseInt(num_individuals, 10) : null,
      individual_names: individual_names || null,
      actual_bill_amount: actual_bill_amount ? parseFloat(actual_bill_amount) : null
    };
    ...
    if (expense_type === "Petrol") {
      const petrolRate = vehicle_type === "2 Wheeler" ? 3.25 : 8.50;
      dynamicDetailsHtml = `
        <tr>
          <td colspan="2" style="padding: 15px 0; border-bottom: 1px solid #f1f5f9;">
            <div style="background-color: #f8fafc; padding: 12px; border-radius: 8px; border: 1px solid #e2e8f0; font-family: sans-serif;">
              <strong style="color: #0f172a; font-size: 13px; display: block; margin-bottom: 6px;">Petrol Mileage Details:</strong>
              <div style="color: #475569; font-size: 12px; line-height: 1.6;">
                • <strong>From:</strong> ${from_location || "N/A"}<br/>
                • <strong>To:</strong> ${to_location || "N/A"}<br/>
                • <strong>Vehicle Type:</strong> ${vehicle_type || "N/A"} (₹${petrolRate.toFixed(2)}/km)<br/>
                • <strong>Distance:</strong> ${distance_km || "0"} km
              </div>
            </div>
          </td>
        </tr>
      ...
      const foodRate = meal_type === "Breakfast" ? 80 : 200;
      const allowanceLimit = foodRate * (num_individuals || 1);
      dynamicDetailsHtml = `
        <tr>
          <td colspan="2" style="padding: 15px 0; border-bottom: 1px solid #f1f5f9;">
            <div style="background-color: #f8fafc; padding: 12px; border-radius: 8px; border: 1px solid #e2e8f0; font-family: sans-serif;">
              <strong style="color: #0f172a; font-size: 13px; display: block; margin-bottom: 6px;">Food Allowance Details:</strong>
              <div style="color: #475569; font-size: 12px; line-height: 1.6;">
                • <strong>Meal Type:</strong> ${meal_type || "N/A"} (₹${foodRate}/person)<br/>
                • <strong>Individuals Count:</strong> ${num_individuals || 1}<br/>
                • <strong>Individual Names:</strong> ${individual_names || "N/A"}<br/>
                • <strong>Actual Bill:</strong> ₹${actual_bill_amount || "0.00"}<br/>
                • <strong>Approved Reimbursement:</strong> ₹${formattedAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })} (Allowance limit: ₹${allowanceLimit.toFixed(2)})
              </div>
            </div>
          </td>
        </tr>
...
// Get Google Maps Platform API key securely for frontend Maps SDK initialization
...
// Mark an individual employee's outstanding expenses as Paid/Reimbursed
app.post("/api/admin/mark-employee-paid", requireAdmin, async (req: any, res: any) => {
  try {
    if (!adminSupabase) {
      return res.status(500).json({ error: "Supabase service role client is not initialized" });
    }

    const { employeeId, archiveName } = req.body;
    if (!employeeId) {
      return res.status(400).json({ error: "Missing required field: employeeId" });
    }

    // 1. Fetch unarchived (unpaid) expenses for this employee
    const { data: activeExpenses, error: fetchErr } = await adminSupabase
      .from("expenses")
      .select("*")
    ...
    if (!activeExpenses || activeExpenses.length === 0) {
      return res.status(400).json({ error: "This employee has no active unpaid expenses to mark as paid." });
    }

    const sum = Math.round(
      activeExpenses.reduce((s: number, e: any) => s + parseFloat(e.amount || 0), 0) * 100
    ) / 100;

    const now = new Date();
    const defaultName = `Paid - ${now.toLocaleString("default", { month: "long" })} ${now.getFullYear()}`;
    const name = (archiveName || defaultName).trim();

    // 2. Create archive record to preserve payment cycle ledger
    const { data: newArchive, error: archError } = await adminSupabase
      .from("expense_archives")
      .insert({
        employee_id: employeeId,
    ...
    if (archError) {
      return res.status(500).json({ error: "Failed to create payment record: " + archError.message });
    }

    // 3. Mark the expenses as is_archived = true and link archive_id
    const expIds = activeExpenses.map((e: any) => e.id);
    const { error: updateError } = await adminSupabase
      .from("expenses")
      .update({
        is_archived: true,
    ...
    if (updateError) {
      await adminSupabase.from("expense_archives").delete().eq("id", newArchive.id);
      return res.status(500).json({ error: "Failed to update expenses status: " + updateError.message });
    }

    return res.json({
      success: true,
      paidAmount: sum,
      claimCount: expIds.length,
      archiveId: newArchive.id,
      archiveName: name
    });
  } catch (err: any) {
    return res.status(500).json({ error: "Server error during mark as paid: " + err.message });
  }
});
  ...
  try {
    ...
    try {
      ...
      if (item.expense_type === "Petrol") {
        const petrolRate = item.vehicle_type === "2 Wheeler" ? 3.25 : 8.50;
        dynamicDetailsHtml = `
          <tr>
            <td colspan="2" style="padding: 15px 0; border-bottom: 1px solid #f1f5f9;">
              <div style="background-color: #f8fafc; padding: 12px; border-radius: 8px; border: 1px solid #e2e8f0; font-family: sans-serif;">
                <strong style="color: #0f172a; font-size: 13px; display: block; margin-bottom: 6px;">Petrol Mileage Details:</strong>
                <div style="color: #475569; font-size: 12px; line-height: 1.6;">
                  • <strong>From:</strong> ${item.from_location || "N/A"}<br/>
                  • <strong>To:</strong> ${item.to_location || "N/A"}<br/>
                  • <strong>Vehicle Type:</strong> ${item.vehicle_type || "N/A"} (₹${petrolRate.toFixed(2)}/km)<br/>
                  • <strong>Distance:</strong> ${item.distance_km || "0"} km
                </div>
              </div>
            </td>
          </tr>
        ...
        const foodRate = item.meal_type === "Breakfast" ? 80 : 200;
        const allowanceLimit = foodRate * (item.num_individuals || 1);
        dynamicDetailsHtml = `
          <tr>
            <td colspan="2" style="padding: 15px 0; border-bottom: 1px solid #f1f5f9;">
              <div style="background-color: #f8fafc; padding: 12px; border-radius: 8px; border: 1px solid #e2e8f0; font-family: sans-serif;">
                <strong style="color: #0f172a; font-size: 13px; display: block; margin-bottom: 6px;">Food Allowance Details:</strong>
                <div style="color: #475569; font-size: 12px; line-height: 1.6;">
                  • <strong>Meal Type:</strong> ${item.meal_type || "N/A"} (₹${foodRate}/person)<br/>
                  • <strong>Individuals Count:</strong> ${item.num_individuals || 1}<br/>
                  • <strong>Individual Names:</strong> ${item.individual_names || "N/A"}<br/>
                  • <strong>Actual Bill:</strong> ₹${item.actual_bill_amount || "0.00"}<br/>
                  • <strong>Approved Reimbursement:</strong> ₹${formattedAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })} (Allowance limit: ₹${allowanceLimit.toFixed(2)})
                </div>
              </div>
            </td>
